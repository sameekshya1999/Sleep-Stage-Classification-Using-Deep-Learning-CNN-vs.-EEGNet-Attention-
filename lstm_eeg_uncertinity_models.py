...
# Monte Carlo Dropout Version (for both LSTM and EEGNet)
...
def predict_with_deep_ensembles(ensembles, X):
    preds = [model(X) for model in ensembles]
    preds = np.stack(preds, axis=0)
    mean_pred = np.mean(preds, axis=0)
    uncertainty = np.std(preds, axis=0)
    return mean_pred, uncertainty
...
def predict_with_variational(model, X, n_samples=50):
...
def predict_with_evidential(model, X):
    evidence = model(X)
    alpha = evidence + 1
    S = tf.reduce_sum(alpha, axis=1, keepdims=True)
    mean_pred = alpha / S
    uncertainty = tf.cast(nb_classes, tf.float32) / S
...
def run_pipeline():
    ...
    uncertainty_methods = [
        ...
        ('Deep_Ensembles', train_deep_ensembles, predict_with_deep_ensembles),
        ...
        ('Evidential_DL', build_evidential_model, predict_with_evidential)
    ]
